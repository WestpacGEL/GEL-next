'use client';

import { clsx } from 'clsx';
import React from 'react';

import { Pictogram } from '../pictogram.component.js';
import { fill } from '../pictogram.styles.js';
import { type PictogramProps } from '../pictogram.types.js';

export function GymPictogram({
  mode = 'duo',
  viewBoxWidth = 78,
  viewBoxHeight = 78,
  'aria-label': ariaLabel = 'Gym',
  copyrightYear = '2021',
  className,
  ...props
}: PictogramProps) {
  return (
    <Pictogram
      className={clsx('size-13', className)}
      viewBoxWidth={viewBoxWidth}
      viewBoxHeight={viewBoxHeight}
      aria-label={ariaLabel}
      copyrightYear={copyrightYear}
      {...props}
    >
      <g fill="none" fillRule="evenodd">
        <polygon
          className={fill({ mode, highlight: true })}
          points="24.855 57.164 24.855 40.328 28.081 40.328 28.081 49.263 31.86 44.968 35.834 44.968 31.666 49.424 36.133 57.164 32.653 57.164 29.585 51.686 28.081 53.26 28.081 57.164"
        />
        <path
          className={fill({ mode, highlight: true })}
          d="M40.575 50.813c0 1.294.25 2.241.752 2.843.5.6 1.12.901 1.855.901.789 0 1.455-.308 1.998-.925.544-.615.816-1.529.816-2.738 0-1.264-.26-2.201-.781-2.813-.521-.614-1.18-.92-1.975-.92-.774 0-1.412.3-1.913.902-.501.601-.752 1.518-.752 2.75m-2.883 7.155l3.687.448c.06.428.203.724.425.885.306.23.788.344 1.447.344.842 0 1.474-.127 1.895-.379.283-.169.498-.44.643-.816.099-.267.149-.762.149-1.48v-1.782c-.965 1.319-2.182 1.976-3.652 1.976-1.638 0-2.936-.693-3.893-2.078-.751-1.094-1.126-2.459-1.126-4.088 0-2.045.493-3.606 1.476-4.686.984-1.08 2.207-1.62 3.669-1.62 1.509 0 2.753.663 3.733 1.986v-1.71h3.02v10.944c0 1.44-.118 2.516-.356 3.227-.237.712-.57 1.272-1 1.677-.428.405-1 .724-1.716.953-.716.23-1.621.346-2.716.346-2.067 0-3.533-.355-4.398-1.064-.866-.708-1.298-1.605-1.298-2.693 0-.106.004-.236.01-.39"
        />
        <path
          className={fill({ mode, outline: true })}
          d="M46.712 18.294l-2.289 8.39a24.81 24.81 0 0 0-9.169-1.117 24.96 24.96 0 0 0-1.498.142l-2.023-7.415c-.187-.686-.076-1.386.297-1.872.133-.175.235-.229.251-.233h13.88c.02.006.121.058.255.233.372.488.483 1.188.296 1.872m-.548-4.604h-18.61c-.845 0-1.64.43-2.238 1.213-.843 1.103-1.112 2.617-.722 4.048l3.005 11.014 1.287-.488a22.267 22.267 0 0 1 6.525-1.413 22.296 22.296 0 0 1 9.42 1.41l1.289.491 3.004-11.012c.391-1.433.121-2.947-.722-4.048-.598-.784-1.393-1.215-2.238-1.215"
        />
        <path
          className={fill({ mode, outline: true })}
          d="M46.069 69.847H27.637a7.157 7.157 0 0 1-5.151-2.202c-4.215-4.41-8.306-12.171-7.973-18.473a22.437 22.437 0 0 1 7.865-15.861l.607-.515-3.404-12.478c-.812-2.983-.213-6.184 1.605-8.565 1.58-2.067 3.898-3.253 6.362-3.253h18.61c2.464 0 4.782 1.186 6.362 3.253 1.818 2.38 2.418 5.582 1.605 8.565L50.718 32.81l.603.514a22.46 22.46 0 0 1 7.904 17.065c0 5.809-4.012 13.062-7.975 17.224a7.193 7.193 0 0 1-5.181 2.234zm12.208-37.898l2.994-10.974c1.016-3.73.257-7.744-2.031-10.74C57.185 7.545 54.142 6 50.892 6H27.554c-3.25 0-6.292 1.544-8.348 4.236-2.288 2.996-3.047 7.01-2.03 10.739l2.989 10.958a24.953 24.953 0 0 0-8.142 17.107c-.388 7.337 4.218 15.683 8.663 20.333a9.67 9.67 0 0 0 6.958 2.974h23.159a9.706 9.706 0 0 0 6.992-3.01c4.306-4.522 8.664-12.483 8.664-18.948a24.965 24.965 0 0 0-8.182-18.44z"
        />
      </g>
    </Pictogram>
  );
}
