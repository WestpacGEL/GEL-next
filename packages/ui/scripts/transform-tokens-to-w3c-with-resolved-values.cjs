...
const primaryTokens = transformPrimitiveDesignTokens(tokens.find(token => token['Primitives']));
const modeTokens = transformDesignTokens(tokens.find(token => token['Tokens']));
const themes = transformThemesDesignTokens(tokens.find(token => token['Themes']));
